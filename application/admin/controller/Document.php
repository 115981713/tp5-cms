...
class Document extends Base {
	...
	public function index() {
		...
		if ($category_id) {
			...
			$map['a.category_id'] = array('in', $category_id.','.$categoryInfo['child']);
		}
		...
		//分类列表
		$categorylist = db('document_category') -> where('status','gt',-1) -> field('id,title,status,pid') -> select();
	...
	public function add_document() {
		if ( request() -> isPost()) {
			...
			if ($re1) {
				...
				$re2 = db('document_article') -> insert($documentExtData);
		...
		if ( request() -> isPost()) {
			...
			if ($re1) {
				...
				if ($docArticle) {
					...
					$documentExtData['content'] = $data['content'];
					$re2 = db('document_article') -> update($documentExtData);
				} else {
					//附表添加数据
					$documentExtData['id'] = $data['id'];
					$documentExtData['content'] = $data['content'];
	...
	public function del_document() {
		...
		$res1 = db('document')->where('id','in',$ids) -> update(['status'=>-1]);
		if ($res1) {
