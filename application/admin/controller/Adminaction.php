...
class Adminaction extends Base
{
    ...
    public function action(){
        ...
        $lists   =   db('admin_action')->where($map)->order('id desc')->paginate(config('LIST_ROWS'),false,['query' => request()->param()]);
    ...
    public function edit_action($id = 0){
        ...
        if(request()->isPost()){
            ...
            $checkNwhere['name']=$data['name'];
            $checkNwhere['status']=array('gt',-1);
            $checkNwhere['id']= array('neq',$data['id']);
            $checkName=db('admin_action')->where($checkNwhere)->find();
            if($checkName){
                $this->error('行为标识重复！');
            }
            ...
            $checkTwhere['title']=$data['title'];
            $checkTwhere['status']=array('gt',-1);
            $checkTwhere['id']= array('neq',$data['id']);
            $checkTitle=db('admin_action')->where($checkTwhere)->find();
            if($checkTitle){
                $this->error('行为名称重复！');
            }
