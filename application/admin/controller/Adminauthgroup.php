...
class Adminauthgroup extends Base
{
    ...
    public function index(){
        ...
        $lists = db('admin_auth_group')->where($map)->paginate(config('LIST_ROWS'),false,['query' => request()->param()]);
...
//            判断用户组名称是否重复
            $checkwhere['title']=$data['title'];
            $checkwhere['status']=array('gt',-1);
            $checkwhere['id']= array('neq',$data['id']);
            $checkTitle=db('admin_auth_group')->where($checkwhere)->find();
            if($checkTitle){
                $this->error('用户组名称重复！');
            }
...
//                添加行为记录
    ...
    public function user($id){
        if (request()->isPost()){
            ...
            $lists=db('admin_member')->where('group_id',$id)->where('status','gt',-1)->select();
